...
namespace AuthServer.Persistence.Services
{
    public class RoleService : IRoleService
    {
        ...
        {
            List<RoleDto> roles = await Task.Run(() => _roleManager.Roles.Select(x => new RoleDto { Id = x.Id, Name = x.Name }).ToList());
        ...
        {
            RoleDto? role = await Task.Run(() => _roleManager.Roles.Where(x => x.Id == roleId).Select(x => new RoleDto { Id = x.Id, Name = x.Name }).FirstOrDefault());
            if (role != null)
        ...
        public async Task<ApiResponse<NoContentDto>> CreateRoleAsync(string roleName)
        {
            ...
            if (result.Succeeded)
                ...
                return ApiResponse<NoContentDto>.Fail(new ErrorDto(result.Errors.Select(x => x.Description).ToList()), StatusCodes.Status400BadRequest);
        }
        ...
        {
            AppUser? user = await _context.Users.FindAsync(assignRoleToUserDto.UserId) ?? throw new Exception("User not found");

            foreach (var userRole in assignRoleToUserDto.RoleIds)
            {
                AppRole? role = await _roleManager.FindByIdAsync(userRole.ToString()) ?? throw new Exception("Role not found");
        ...
        public async Task<ApiResponse<NoContentDto>> AssignRoleToEndpoint(AssignRoleToEndpointDto assignRoleToEndpointDto)
        {
            ...
            foreach (var roleName in assignRoleToEndpointDto.Roles)
            {
                ...
                if (!endpoint.Roles.Any(r => r.Id == role.Id))
                {
        ...
        public async Task<ApiResponse<NoContentDto>> AssignRoleToEndpoints(AssignRoleToEndpointsDto assignRoleToEndpointsDto)
        {
            foreach (var endpointCode in assignRoleToEndpointsDto.EndpointCodes)
            {
                await AssignRoleToEndpoint(new AssignRoleToEndpointDto() { Code = endpointCode, Roles = assignRoleToEndpointsDto.Roles });
            }
