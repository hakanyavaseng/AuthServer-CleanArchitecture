...
namespace AuthServer.Persistence.Services
{
    public class AuthorizationService : IAuthorizationService
    {
            ...
            foreach (var controller in controllers)
            {
                ...
                foreach (var action in actions)
                {
                    ...
                    ActionDto actionDto = new()
                    {
                        ...
                        Code = $"{authorizeDefinitionAttribute.Menu}.{httpMethodAttribute?.HttpMethods.First() ?? HttpMethods.Get}.{Enum.GetName(authorizeDefinitionAttribute.ActionType)}.{action.Name}"
                    };
        ...
        public async Task<ApiResponse<MenuDto>> RegisterAuthorizeDefinitionEndpointAsync(RegisterEndpointsDto registerEndpointsDto, Type type, CancellationToken cancellationToken)
        {
        ...
        public async Task<ApiResponse<NoContentDto>> SaveEndpointsAsync(List<MenuDto> menus)
        {
            ...
            var endpointsToRemove = await _context.AuthEndpoints.Where(e => removedEndpointIds.Contains(e.Id)).ToListAsync();
            ...
            foreach (var menuDto in menus)
            {
                ...
                var menu = await _context.AuthMenus.Include(m => m.AuthEndpoints).FirstOrDefaultAsync(m => m.Name == menuDto.Name);

                if (menu == null)
                {
                    // If the menu doesn't exist, create it and add it to the context
                    menu = new AuthMenu { Id = Guid.NewGuid(), Name = menuDto.Name, AuthEndpoints = new List<AuthEndpoint>() };
